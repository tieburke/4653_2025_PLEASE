...
public class SwerveModule extends SubsystemBase {
    ...
    private void setSpeedPP(SwerveModuleState desiredState, boolean isOpenLoop) {
        if (isOpenLoop) {
            ...
            mDriveMotor.getClosedLoopController().setReference(desiredState.speedMetersPerSecond, ControlType.kVelocity,
                    ClosedLoopSlot.kSlot0);// , feedforward.calculate(desiredState.speedMetersPerSecond));
    ...
    public Rotation2d getAbsoluteAngle() {
        ...
        // {
        // return
        // Rotation2d.fromDegrees(absoluteEncoder.getAbsolutePosition().getValueAsDouble()*360);
        // }
    }
    ...
    public void configAngleMotor() {
        ...
        mAngleConfig.encoder.positionConversionFactor((1 / Constants.Swerve.chosenModule.angleGearRatio) // We do 1 over
                                                                                                         // the gear
                                                                                                         // ratio
                                                                                                         // because 1
                                                                                                         // rotation of
    ...
    public Rotation2d getCanCoder() {
        ...
        // }
        // else{
        // SmartDashboard.putNumber("ctre_angle",
    ...
    public SwerveModulePosition getPosition() {
        for (int i = 0; i < 4; i++) {
            ...
            SmartDashboard.putNumber("Position of Encoder" + moduleNumber,
                    Conversions.neoToMeters(mDriveEncoder.getPosition(), Constants.Swerve.wheelCircumference,
                            Constants.Swerve.driveGearRatio, moduleNumber));
        }

        return new SwerveModulePosition(
                // Originally was mDriveEncoder.getPosition()*360
                (Conversions.neoToMeters(mDriveEncoder.getPosition() * 4096, Constants.Swerve.wheelCircumference,
                        Constants.Swerve.driveGearRatio, moduleNumber)),
                getAngle());
    }

    public void getItRight() {
        mAngleMotor.getClosedLoopController().setReference(
                (mAngleEncoder.getPosition() % 360) + (getAbsoluteAngle().getDegrees()), ControlType.kPosition);
    }
